...
class RollTracker { 
    ...
    static FLAGS = {
        //Array of DIE_INFO Values
        D2_INFO:     new DIE_INFO(DIE_TYPE.D2),
        D4_INFO:     new DIE_INFO(DIE_TYPE.D4),
        D6_INFO:     new DIE_INFO(DIE_TYPE.D6),
        D8_INFO:     new DIE_INFO(DIE_TYPE.D8),
        D10_INFO:    new DIE_INFO(DIE_TYPE.D10),
        D12_INFO:    new DIE_INFO(DIE_TYPE.D12),
        D20_INFO:    new DIE_INFO(DIE_TYPE.D20),
        D100_INFO:   new DIE_INFO(DIE_TYPE.D100),
...
// Our main data workhorse class
    ...
    // A simple retrieve method that gets the stored flag on a specified user
        const output = {
            ...
            sorted: game.users.get(userId)?.getFlag(RollTracker.ID, RollTracker.FLAGS.D2_INFO),
            unsorted: game.users.get(userId)?.getFlag(RollTracker.ID, RollTracker.FLAGS.UNSORTED_D20),
            export: game.users.get(userId)?.getFlag(RollTracker.ID, RollTracker.FLAGS.EXPORT_D20),
            streak: game.users.get(userId)?.getFlag(RollTracker.ID, RollTracker.FLAGS.STREAK_D20),
        } 
        return output
    }
    ...
    static createTrackedRoll(user, rollData, isBlind) {
        ...
        // roll is the foundry instance of the user actually making the roll
            ...
            if (oldSorted.length) {
                ...
                game.users.get(user.id)?.setFlag(RollTracker.ID, RollTracker.FLAGS.STREAK_D20, streak)
            } else {
                updatedRolls = newNumbers
                oldUnsorted = newNumbers
            }
            return Promise.all([
                game.users.get(user.id)?.setFlag(RollTracker.ID, RollTracker.FLAGS.SORTED_D20, updatedRolls),
                game.users.get(user.id)?.setFlag(RollTracker.ID, RollTracker.FLAGS.UNSORTED_D20, oldUnsorted)
            ])
        }
    }
    ...
    // Delete all stored rolls for a specified user ID
        return Promise.all([
            game.users.get(userId)?.unsetFlag(RollTracker.ID, RollTracker.FLAGS.SORTED_D20), 
            game.users.get(userId)?.unsetFlag(RollTracker.ID, RollTracker.FLAGS.EXPORT_D20),
            game.users.get(userId)?.unsetFlag(RollTracker.ID, RollTracker.FLAGS.UNSORTED_D20),
            game.users.get(userId)?.unsetFlag(RollTracker.ID, RollTracker.FLAGS.STREAK_D20),
        ])
    }
    ...
    // prepare the roll data for export to an R-friendly text file
        ...
        game.users.get(game.userId)?.setFlag(RollTracker.ID, RollTracker.FLAGS.EXPORT_D20, fileContent)
    }
    ...
    static async generalComparison() {
        let allStats = {}
        for (let user of game.users) {
            if (game.users.get(user.id)?.getFlag(RollTracker.ID, RollTracker.FLAGS.SORTED_D20)) {
